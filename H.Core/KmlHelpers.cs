...
namespace H.Core
{
    ...
    public class KmlHelpers
    {
        ...
        private async Task LoadAllPolygonsFromKmlFilesAsync()
        {
            await Task.Run(() =>
            {
                // KML files are embedded resources in the H.Infrastructure assembly
                var infraAssembly = typeof(H.Infrastructure.InfrastructureConstants).Assembly;
                foreach (var entry in _fileNames)
                {
                    var polygonList = new List<(int polygonId, Polygon sharpKmlPolygon)>();
                    var fileName = $@"{infraAssembly.GetName().Name}.Resources.{entry.FileName}";
                    using var stream = infraAssembly.GetManifestResourceStream(fileName);
